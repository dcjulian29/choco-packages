...
# Remove "default-added" extensions I don't want.
$packages  = [string[]](Get-Content "$PSScriptRoot\extensions.remove")
foreach ($package in $packages) {
  Start-Process -FilePath $code -ArgumentList "--uninstall-extension $package --force" -NoNewWindow -Wait
}
